...
#[tauri::command]
pub fn connnect(serial: String) -> Result<(), String> {
    let mut core = core_instance().lock().unwrap();
    let connected_aah = AAH::connect(serial)?;
...
pub fn get_screen() -> Result<Vec<u8>, String> {
    let core = core_instance().lock().unwrap();
    if core.is_none() {
    ...
    let mut buf = Vec::new();
    screen.write_to(&mut Cursor::new(&mut buf), ImageFormat::Bmp).map_err(|e| format!("编码图像失败: {:?}", e))?;
